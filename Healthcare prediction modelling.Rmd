...
pacman::p_load(plyr, broom, kableExtra, rstatix, rstatix, tidyverse, ggmosaic, DT, rpart, rpart.plot, glmnet, randomForest, ranger)
...
Still, a noticeable number of patients were charged above 10k, with even 5% reaching more than 28k dollars.
...
Simplifying things, the smaller the charge, the higher the chance of return. 
However, the rates of return were relatively similar for patients who were charged between 1k and 28k dollars. The biggest differences were observed for those who either were charged very little (up to 1k) or a lot (above 28k).
...
**Dodać odds ratio confidence interval**
...
healthcare3 %>% 
  ggplot() +
  geom_mosaic(aes(x = product(diagnosis, return, severity), fill = return)) +
  coord_flip()
...
predict(fit_tree, newdata = validation, type = "class")
...
df_trees2 <- df_trees %>% 
  mutate(
    pred_validation = map(fit_train, predict, type = "class", newdata = validation),
    miss_error = map_dbl(pred_validation, function(x) {
      mean(x != validation$return)
    })
  ) %>% 
  select(name, value, miss_error) %>% 
  set_names(c("Model", "Complexity Parameter", "validation classification error")) 

best_tree_miss <- min(df_trees2$`validation classification error`)
...
# set.seed(321)
# 
# randomForest(return ~ ., data = train, ntree = 1000)
# 
# 
# forests <- tibble(mtry = 8:(ncol(train) - 1)) %>% 
#   mutate(
...
#         data = train, 
#         mtry = x,
#         ntree = 100 # To safe some computing power
#         )})
#   )
...
#### Ranger package for parallel computing

Porównać przy pomocy funkcji pod względem speed
Dodać również zabawę z num trees, ale po pomniejszeniu liczby predyktorów poprzez kategoryzację hour, month, weekday
...
The best random forest model was achieved using m = `r forests$mtry[[1]]`.
...
```{r}
X <- model.matrix(return ~ ., data = train)[,-1]
Y <- train$return
...
Consider to first recode time variables to a smaller number of levels, especially hour.


```{r}
# # Extract obtained interactions to a formula like vector
# interactions_formula <- interactions2 %>% 
#   pull(variable) %>% 
#   str_replace(":return", "") 
#   
# 
# # Create formula for the model with interactions
# int_formula <- healthcare_final %>% 
#   names() %>% 
#   keep(. != "return") %>% 
#   c(interactions_formula) %>% 
#   str_c(collapse = " + ") %>% 
#   {str_c("return ~ ", .)} %>% 
#   as.formula()
#   
...
# 
# 
# # Build the step model
# fit_step <- step(step_full, c(upper = step_full, lower = step_null), direction = "backward")
```
...
step_predicted <- predict(fit_step, newdata = validation, type = "response") %>% 
  {if_else(. >= 0.5, "Yes", "No")}  
