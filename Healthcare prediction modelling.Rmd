...
Perhaps create code that will create possible combinations of simple interactions.
  ...
  map(
    function(x) {
      as.data.frame(x) %>% 
        ...
        mutate(
        variable = rownames(.),
        dev_prop = deviance / max(resid.dev) * 100
        ) %>% 
        filter(str_detect(variable, ".*:.*:")) %>% 
        select(variable, deviance, dev_prop, p) %>% 
        as_tibble()
...
interactions2 <- interactions %>% 
  filter(p < 0.001) %>% 
  arrange(desc(dev_prop))
```


We can extract odds ratio for the more important interactions

Kluczowy problem - odds ratio mogę obliczać tylko dla tabel 2 x 2, a nie wszystkie takei są

Rozwiązania

-   Cramer's V
-   Uwaga - zmienic kolejnosc kolumn dla kazdego porowniania tak, zeby jako pierwsza byla zmienna o większej liczbie poziomów



```{r}
# Create odds ration function
...
interactions2 %>% 
  ...
  map(
    function(x) {
      select(healthcare3, all_of(x)) %>% 
        nest(.by = 1) %>% 
        mutate(variable = names(.)[1]) %>% 
        set_names(c("levels", "data", "variable")) 
      }
    ) %>% 
  reduce(add_row) %>% 
  select(variable, levels, data) %>% 
  mutate(
...
1.    Estimate effect sizes for interaction effects
2.    Extract only those which are above a certain effect size


```{r}
health_list %>% 
  pivot_longer(-c(2, return)) %>% 
  nest(.by = name) %>% 
  mutate(
    freq = map(data, freq_table, gender, value, return)
  ) %>% 
  unnest(freq) 
