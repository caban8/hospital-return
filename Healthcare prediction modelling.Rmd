...
pacman::p_load(plyr, broom, kableExtra, rstatix, rstatix, tidyverse, ggmosaic, DT, rpart, rpart.plot, glmnet, randomForest, ranger, gbm, xgboost)
...
xgboost_models2 <- xgboost_models %>% 
  mutate(
    predictions = map(model, predict, newdata = X_validation),
    class = map(predictions, function(x) {if_else(x > 0.5, 1, 0)}),
    miss_error = map_dbl(class, function(x) {mean(x != Y_validation)}),
    miss_error = round(miss_error, 3)
    ) %>% 
  arrange(miss_error) %>% 
...
Analyzing the importance of the predictors for the model with the smallest validation classification error, age and charges were observed to have the highest contribution to the prediction.

Of significant importance were also gender and ed result.
...
step_predicted <- predict(step_full, newdata = validation, type = "response") %>% 
  {if_else(. >= 0.5, "Yes", "No")}  
