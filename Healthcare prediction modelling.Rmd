...
pacman::p_load(plyr, broom, kableExtra, rstatix, rstatix, tidyverse, ggmosaic, DT, rpart, rpart.plot, glmnet)
...
predict(fit_tree, newdata = test, type = "class")
...
# Compare miss error on the test set
df_trees %>% 
  mutate(
    pred_test = map(fit_train, predict, type = "class", newdata = test),
    miss_error = map_dbl(pred_test, function(x) {
      mean(x != test$return)
    })
  )
